...
export default function CalendarView({ bookings = [], onSelectBooking, onViewSlip, onUpdateStatus }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-7 bg-white rounded-2xl border border-stone-200 p-5 shadow-sm">
        {/* Month Header */}
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex gap-1">
            ...
            <button
              ...
              className="px-3 py-1 text-xs font-semibold text-stone-700 hover:bg-stone-100 rounded-lg transition-colors border border-stone-200"
            >
              วันนี้
            </button>
            <button
        ...
        <div className="grid grid-cols-7 gap-1.5">
          {days.map((day, idx) => {
            if (!day) {
              return <div key={`empty-${idx}`} className="h-20 bg-stone-50/50 rounded-xl" />;
            }
              ...
              >
                ...
                {nonCancelled.length > 0 && (
                  <div className="space-y-0.5 overflow-hidden">
                    <span className="block text-[10px] font-bold text-amber-700 bg-amber-100/80 px-1 py-0.5 rounded leading-tight text-center">
      ...
      <div className="lg:col-span-5 bg-white rounded-2xl border border-stone-200 p-5 shadow-sm flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 max-h-[500px] pr-1">
          {selectedDayBookings.length === 0 ? (
            ...
            selectedDayBookings.map((b) => (
              <div
                key={b.id}
                className="p-3.5 rounded-xl border border-stone-200 hover:border-stone-300 bg-stone-50/70 transition-all space-y-2.5"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs font-bold bg-white px-2 py-0.5 rounded border border-stone-200 text-stone-800">
                      {b.time} น.
                    </span>
                    <span className="text-xs font-bold text-stone-800">{b.serviceName}</span>
                  </div>
                ...
                <div className="flex items-center justify-between text-xs pt-1 border-t border-stone-200/60">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <select
                      ...
                      className="text-[11px] font-semibold bg-white border border-stone-300 rounded-md px-2 py-1 text-stone-700 focus:outline-none focus:ring-1 focus:ring-stone-800"
                    >
